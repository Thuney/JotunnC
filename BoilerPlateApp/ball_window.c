...
static void ball_init(
  struct ball_t* ball)
{
  ball->color = (fvector4){1.0f, 0.0f, 0.0f, 1.0f};

  ball->radius       = 20.0f;
  ball->position     = (fvector2){200.0f, 200.0f};
  ball->velocity     = (fvector2){0.0f, 10.0f};
  ball->acceleration = (fvector2){0.0f, -800.0f};

  ball->held = false;
  ball->grounded = false;
}

uint8_t ball_window_init(
  struct application_t* app_parent,
  struct ball_window_t* ball_window)
{
  uint8_t error = 0U;
  ...
  struct camera_base_t* camera_base_ptr =
    (struct camera_base_t*)&(ball_window->camera_ortho);
  struct renderer_base_t* renderer_base_ptr =
    (struct renderer_base_t*)&(ball_window->renderer_2d);
  ...
  if (!error)
  {
    ...
    renderer_2d_init(
      &(ball_window->renderer_2d),
      camera_base_ptr,
      window_ptr,
      "ball_app_renderer_2d");

    framebuffer_init(
      &(ball_window->framebuffer),
      width,
      height);

    window_layer_init(
      window_ptr,
      &(ball_window->window_layer),
      &(ball_window->framebuffer),
      camera_base_ptr,
      renderer_base_ptr);
    ...
    window_add_layer(
      &(ball_window->window),
      &(ball_window->ball_ui_layer.ui_layer.ui_window_layer));

    window_bind_custom_events(
      window_ptr,
      &ball_window_on_event);
...
{
  fvector2 *pos   = &(ball->position);
  fvector2 *vel   = &(ball->velocity);
  fvector2 *accel = &(ball->acceleration);


  if (fabs(vel->comp.x) <= 0.1)
  {
    vel->comp.x = 0.0f;
  }

  if (fabs(vel->comp.y) <= 0.1 && !ball->grounded)
  {
    vel->comp.y = 0.0f;
    ball->grounded = true;
  }
  ...
  vel->comp.x +=
    (accel->comp.x * delta_time_seconds);
  vel->comp.y +=
    (!ball->grounded) * (accel->comp.y * delta_time_seconds);

  float new_x = pos->comp.x +
    (vel->comp.x * delta_time_seconds) +
    (0.5f) *
      (accel->comp.x *
       (delta_time_seconds * delta_time_seconds));
  float new_y = pos->comp.y +
    (vel->comp.y * delta_time_seconds) +
    (!ball->grounded) *
      ((0.5f) *
       (accel->comp.y *
        (delta_time_seconds * delta_time_seconds)));
  ...
  if (((new_x - ball->radius) <= 0) ||
      ((new_x + (ball->radius)) >= window_width))
  {
    vel->comp.x *= -0.6f;
    new_x = pos->comp.x +
      (vel->comp.x * delta_time_seconds) +
  ...
  if (((new_y - ball->radius) <= 0) ||
      ((new_y + ball->radius) >= window_height))
  {
    vel->comp.y *= -0.6f;
    new_y = pos->comp.y +
      (vel->comp.y * delta_time_seconds) +
  ...
  pos->comp.x = new_x;
  pos->comp.y =
    ((new_y - ball->radius) <= 0) ? (ball->radius) : (new_y);
}

static bool mouse_intersects_ball(
  const fvector2 mouse_pos,
  struct ball_t* ball,
  fvector2* mouse_diff)
{
  const fvector2 ball_center =
    (fvector2){(ball->position.comp.x), (ball->position.comp.y)};
  ...
  return (hypotenuse < ball->radius);
}

void ball_window_on_event(
  ...
  struct event_base_t* event)
{
  ...
  switch (event->event_type)
  {
    case EVENT_APP_TICK:
    {
      ...
      if (!ball->held)
      {
        ball_update(
          ball,
          delta_time_seconds,
          ball_window->window.metadata.width,
          ball_window->window.metadata.height);
      }
    ...
    {
      struct event_mouse_moved_t* event_mouse_moved =
        (struct event_mouse_moved_t*)event;
      ...
      if (ball->held)
      {
        ball->position.comp.x =
          (cur_mouse.comp.x - intersection_diff.comp.x);
        ball->position.comp.y =
          (cur_mouse.comp.y - intersection_diff.comp.y);
      }
    }
    break;
    ...
    {
      struct event_mouse_button_t* event_mouse_button =
        (struct event_mouse_button_t*)event;
        ...
        switch (event_mouse_button->action)
        {
          ...
          case 1:
          {
            ...
            if (mouse_intersects_ball(cur_mouse, ball, &intersection_diff))
            {
              ...
              ball->velocity.comp.x = 0.0f;
              ball->velocity.comp.y = 0.0f;
            }
          }
          break;
          // Released
          case 0:
          {
            if (ball->held)
            {
              ...
              ball->velocity.comp.x = (delta_mouse.comp.x * 300.0f);
              ball->velocity.comp.y = (delta_mouse.comp.y * 300.0f);
            }
          }
          break;
        }
...
static const fmatrix_4x4 ball_calculate_transform(
  struct ball_t* ball)
{
  fmatrix_4x4 transform_matrix;
  {
    ...
    const fvector3 scale_factors =
      (fvector3){{ball->radius, ball->radius, ball->radius}};
    const fvector3 translation_vector = (fvector3){
      {ball->position.comp.x, ball->position.comp.y, 0.0f}};
    ...
    translation_matrix =
      fmatrix_4x4_transform_translate(&(translation_matrix), translation_vector);
    transform_matrix = fmatrix_4x4_multiply(&(scale_matrix), &(translation_matrix));
  }
  return transform_matrix;
}

static const fmatrix_4x4 click_calculate_transform(
  struct ball_window_t* ball_window)
{
  fmatrix_4x4 transform_matrix;
  {
    ...
    const fvector3 scale_factors =
      (fvector3){{ball_window->clicked_indicator_radius,
                  ball_window->clicked_indicator_radius,
                  ball_window->clicked_indicator_radius}};
    const fvector3 translation_vector =
      (fvector3){{ball_window->last_clicked.comp.x,
                  ball_window->last_clicked.comp.y,
                  0.0f}};
...
void ball_window_run(
  struct window_layer_t* window_layer)
{
  struct ball_window_t* ball_window =
    (struct ball_window_t*)window_layer->parent_window;

  const fmatrix_4x4 ball_transform =
    ball_calculate_transform(&(ball_window->ball));
  ...
  renderer_2d_draw_circle(
    &(ball_window->renderer_2d),
    &(ball_transform),
    ball_window->ball.color);

  renderer_2d_draw_circle(
    &(ball_window->renderer_2d),
    &(click_transform),
    ball_window->clicked_color);
}

void ball_ui_window_run(
