...
struct window_layer_t
{
    ...
    void (*function_custom_window_layer_run)(struct window_t* parent_window, struct window_layer_t* window_layer);
    // Acts on events propagated through to this layer
    void (*function_custom_event)(struct window_t* parent_window, struct window_layer_t* window_layer, struct event_base_t* event);
};
