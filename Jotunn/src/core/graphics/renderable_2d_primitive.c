...
{
  const float angle_increment_degrees =
    (360.0f / (float)_RENDERABLE_2D_CIRCLE_RESOLUTION);
  ...
  circle_2d_position_data[0] =
    (fvector3){offset.comp.x, offset.comp.y, 0.0f}; // Origin / Center of circle
...
static struct vertex_attribute_t textured_quad_vertex_attributes[4] =
{
  ...
  [3] =
  {
    ...
    .should_normalize = 0,
    .stride = sizeof(struct renderable_2d_textured_quad_vertex_t),
    .ptr_offset_to_attrib =
      (void*)(sizeof(fvector3) + sizeof(fvector2) + sizeof(float))
  }
};
  ...
  struct shader_program_t* shader_program)
{
  ...
  for (attribute_index = 0; attribute_index < num_attributes; attribute_index++)
  {
    ...
    temp_shader_index =
      shader_program_get_attribute_location(shader_program, attribute_name);
  ...
  struct shader_program_t* shader_program)
{
  ...
  renderable_2d_primitive_data_init(
    &triangle_data->vao,
    &triangle_data->vbo,
    &triangle_data->ebo,
    1,
  ...
  triangle_data->vertex_data_base =
    (struct renderable_2d_triangle_vertex_t*)calloc(
      max_vertices, sizeof(struct renderable_2d_triangle_vertex_t));
  ...
  element_buffer_buffer_data(
    &triangle_data->ebo,
    triangle_indices,
    max_indices * sizeof(unsigned int),
    STATIC_DRAW);
  vertex_buffer_buffer_data(
    &triangle_data->vbo,
    (float*)triangle_data->vertex_data_base,
    max_vertices * sizeof(struct renderable_2d_triangle_vertex_t),
    DYNAMIC_DRAW);
  vertex_array_unbind();
}
  ...
  struct shader_program_t* shader_program)
{
  ...
  renderable_2d_primitive_data_init(
    ...
    &quad_data->ebo,
    1,
    quad_vertex_attributes,
    _QUAD_2D_ATTRIBUTE_NUM,
    shader_program);

  quad_data->vertex_data_base = (struct renderable_2d_quad_vertex_t*)calloc(
    max_vertices, sizeof(struct renderable_2d_quad_vertex_t));
  ...
  vertex_buffer_buffer_data(
    &quad_data->vbo,
    (float*)quad_data->vertex_data_base,
    max_vertices * sizeof(struct renderable_2d_quad_vertex_t),
    DYNAMIC_DRAW);
  ...
  struct shader_program_t* shader_program)
{
  ...
  renderable_2d_primitive_data_init(
    &textured_quad_data->vao,
    &textured_quad_data->vbo,
    &textured_quad_data->ebo,
    1,
    textured_quad_vertex_attributes,
    _TEXTURED_QUAD_2D_ATTRIBUTE_NUM,
    shader_program);

  textured_quad_data->vertex_data_base =
    (struct renderable_2d_textured_quad_vertex_t*)calloc(
      max_vertices, sizeof(struct renderable_2d_textured_quad_vertex_t));
  ...
  vertex_buffer_buffer_data(
    &textured_quad_data->vbo,
    (float*)textured_quad_data->vertex_data_base,
    max_vertices * sizeof(struct renderable_2d_textured_quad_data_t),
    DYNAMIC_DRAW);
  ...
  struct shader_program_t* shader_program)
{
  ...
  const unsigned int indices_per_circle =
  ...
  circle_data->vertex_data_base = (struct renderable_2d_circle_vertex_t*)calloc(
    max_vertices, sizeof(struct renderable_2d_circle_vertex_t));
  ...
  for (circle = 0; circle < max_count; circle++)
  {
    ...
    {
      circle_indices[indices_per_circle * circle + (3 * (triangle - 1)) + 0] =
        vertex_offset;
      circle_indices[indices_per_circle * circle + (3 * (triangle - 1)) + 1] =
        vertex_offset + triangle;
      circle_indices[indices_per_circle * circle + (3 * (triangle - 1)) + 2] =
  ...
  vertex_buffer_buffer_data(
    &circle_data->vbo,
    (float*)circle_data->vertex_data_base,
    max_vertices * sizeof(struct renderable_2d_circle_vertex_t),
    DYNAMIC_DRAW);
  ...
  struct shader_program_t* shader_program)
{
  ...
  line_data->vertex_data_base = (struct renderable_2d_line_vertex_t*)calloc(
    max_vertices, sizeof(struct renderable_2d_line_vertex_t));
