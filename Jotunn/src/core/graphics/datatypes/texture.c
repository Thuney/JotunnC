  ...
  bool wrap)
{
  ...
  switch (internal_format)
  {
    ...
    {
      texture->data_format = TEXTURE_2D_FORMAT_RG;
      texture->channels = 2;
    }
    break;
  ...
  const unsigned int flip_vertically)
{
  ...
  texture->data = stbi_load(
    filepath, &texture->width, &texture->height, &texture->channels, 0);
  ...
  {
    texture->is_loaded =
      1; // Indicate that we are already loaded so we don't needlessly re-set fields

    texture_2d_init(
      texture, texture->width, texture->height, texture->internal_format, true);
    texture_2d_set_data(texture, texture->data, texture->data_size_bytes);
  }
}
